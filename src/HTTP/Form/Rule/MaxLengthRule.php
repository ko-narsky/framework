...
final readonly class MaxLengthRule implements FormRequestRuleInterface
{
    ...
    public function validate(mixed $value, mixed $options): void
    {
        ...
        if (mb_strlen($value) > $options) {
            throw new ValidationException('Длина строки не может превышать ' . $options . ' ' . $this->getSymbolWord($options));
        }
    }
